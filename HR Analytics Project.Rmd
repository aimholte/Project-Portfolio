...
library(bestglm)
#Prepare the data.
HR_data_for_best_logistic <- within(HR_data, {
 ...
 salarymedium <- NULL
 salarylow <- NULL
 satisfaction.squared <- NULL
 satisfaction.cubed <- NULL
 satisfaction.4 <- NULL
 satisfaction.5 <- NULL
 avg_hours.squared <- NULL
 avg_hours.cubed <- NULL
 avg_hours.4 <- NULL
 avg_hours.5 <- NULL
 time_spent.squared <- NULL
 time_spent.cubed <- NULL
 time_spent.4 <- NULL
 time_spent.5 <- NULL
 project_hours.int <- NULL
 satisfaction_time_spent.int <- NULL
})

vars.reordered <- c("satisfaction_level", "last_evaluation", "number_project", "average_montly_hours",
                                                          "time_spend_company", "Work_accident", "promotion_last_5years", "salesaccounting", "saleshr",
                                                          "salesIT", "salesmanagement", "salesmarketing", "salesRand", "salessales", "salessupport",
                                                          "salestechnical", "salaryhigh", "salarymedium", "salarylow", "satisfaction.squared",
                                                          "satisfaction.cubed", "satsifaction.4", "satisfaction.5", "avg_hours.squared", "avg_hours.cubed",
                                                          "avg_hours.4", "avg_hours.5", "time_spent.squared", "time_spent.cubed", "time_spent.4",
                                                          "time_spent.5", "project_hours.int", "satisfaction_time_spent.int", "y")

#Re-Order Variables:

HR_data_for_best_logistic <- HR_data_for_best_logistic[,vars.reordered]

#Perform the subset selection
best.glm <- bestglm(Xy = HR_data_for_best_logistic, IC = "AIC", method = "exhaustive")
```
