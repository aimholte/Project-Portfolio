...
summary(glm.mod)
glm.mod2 <- glm(left ~.^2, family = "binomial", data = HR_data)
summary(glm.mod2)
```
```{r}
glm.mod.pred <- predict(glm.mod, HR_data, type = "response")
glm.mod.pred2 <- predict(glm.mod2, HR_data, type = "response")
table(HR_data$left, glm.mod.pred > 0.5)
table(HR_data$left, glm.mod.pred2 > 0.5)
```
